...
class RasterWindow(QWidget):
    ...
    def __init__(self, mainWindow):
        ...
        self.combined_checkbox = QCheckBox(
            "Combined (put neurons of all planes together) ", self
        )
    ...
    def update_day_choice(self):
        if self.day_choice.count() == 1:
            self.all_stat_t2p = (
                self.main_window.central_widget.data_management.all_stat_t2p
            )
            if self.checkbox3.isChecked() or self.checkbox5.isChecked():
                logger.debug(
                    f"Updating day choice combo box with recording indices (count: {self.day_choice.count()})"
                )
    ...
    def get_checkbox_choice(self):
        ...
        if self.checkbox1.isChecked():
            ...
            self.plot_track2p_rasters(
                self.all_f_t2p_preproc, bin_size=self.bin_size, vmin=vmin, vmax=vmax
            )
        if self.checkbox2.isChecked():
            ...
            self.plot_track2p_rasters(
                all_f_t2p_sorted, bin_size=self.bin_size, vmin=vmin, vmax=vmax
            )  # plot the rasters
        if self.checkbox3.isChecked():
            self.raster_type = "sorting_by_PCA_and_by_day_" + str(
                self.day_choice.currentText()
            )
            all_pca_emb_1d = []
            for f in tqdm(self.all_f_t2p_preproc):
                logger.debug(f"Processing raster {f.shape}")
        ...
        if self.checkbox5.isChecked():
            self.raster_type = "sorting_tSNE_and_by_day_" + str(
                self.day_choice.currentText()
            )
            all_tsne_emb_1d = []
            for f in tqdm(self.all_f_t2p_preproc):
    ...
    def preprocessing(self):
        ...
        self.bin_size = int(
            self.bin.text()
        )  # number of frames to average (1 = no averging)
        logger.debug(f"bin_size: {self.bin_size}")
        rem_zero_rows = True
        if bin_data:
            all_f_t2p_original = copy.deepcopy(self.all_f_t2p)
            self.all_f_t2p_preproc = [
                np.mean(f.reshape(f.shape[0], -1, self.bin_size), axis=2)
                for f in all_f_t2p_original
            ]
            # renormalize
        ...
        if rem_zero_rows:
            ...
            zero_rows = np.any(
                [np.sum(np.isnan(f), axis=1) for f in self.all_f_t2p_preproc], axis=0
            )
    ...
    def zscore(self, f, axis=1):
        ...
        return (f - np.nanmean(f, axis=axis, keepdims=True)) / np.nanstd(
            f, axis=axis, keepdims=True
        )

    def zscore_all_f_t2p(self, all_f_t2p):
        return [
            zscore(f, axis=1) for f in all_f_t2p
        ]  # axis=1 means that we are normalizing each neuron's activity (each line of the matrix f)

    def plot_track2p_rasters(self, all_f_t2p, bin_size=1, vmin=None, vmax=None):

        fig, ax = plt.subplots(
            len(all_f_t2p), 1, figsize=(6, len(all_f_t2p) * 1), dpi=150
        )  # rows, columns, size, resolution

        for i, f in enumerate(all_f_t2p):
            ax[i].imshow(
                zscore(f, axis=1), aspect="auto", cmap="Greys", vmin=vmin, vmax=vmax
            )
            ...
            ax[i].set_yticklabels(
                ax[i].get_yticklabels(), rotation=0, va="center", ha="right"
            )
            ...
            if i == len(all_f_t2p) - 1:
                ...
                ax[i].set_xticklabels(
                    [0, int((f.shape[1] * bin_size) / 2), int(f.shape[1] * bin_size)]
                )
        ...
        if track_ops.nplanes > 1:
            ...
            for plane in range(track_ops.nplanes):
                ...
                if plane == self.main_window.central_widget.data_management.plane:
                    logger.debug(
                        "plane is equal to the current plane, skipping to the next plane"
                    )
                ...
                t2p_match_mat_allday = t2p_match_mat[
                    ~np.any(t2p_match_mat == None, axis=1), :
                ]
                trace_type = (
                    self.main_window.central_widget.data_management.trace_type
            ...
            for i in range(num_elements):
                ...
                for plane in range(track_ops.nplanes):
                    if "all_ft2p" in self.results_by_plane[plane] and isinstance(
                        self.results_by_plane[plane]["all_ft2p"], list
                    ):
                        ...
                        logger.debug(
                            "La clé 'all_ft2p' n'existe pas ou n'est pas une liste."
                        )
    ...
    def process_plane(self, plane, track_ops, t2p_match_mat_allday, trace_type):
        ...
        for i, ds_path in enumerate(track_ops.all_ds_path):
            iscell = np.load(
                os.path.join(ds_path, "suite2p", f"plane{plane}", "iscell.npy"),
                allow_pickle=True,
            )
            if trace_type == "F":
                logger.debug("F trace")
                f = np.load(
                    os.path.join(ds_path, "suite2p", f"plane{plane}", "F.npy"),
                    allow_pickle=True,
                )
            if trace_type == "spks":
                logger.debug("spks trace")
                f = np.load(
                    os.path.join(ds_path, "suite2p", f"plane{plane}", "spks.npy"),
                    allow_pickle=True,
                )
            if trace_type == "dF/F0":
                ...
                fneu = np.load(
                    os.path.join(ds_path, "suite2p", f"plane{plane}", "Fneu.npy"),
                    allow_pickle=True,
                )
        ...
        self.results_by_plane[plane] = {"all_ft2p": all_ft2p, "all_fneu2p": all_fneu2p}
