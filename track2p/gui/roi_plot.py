...
class ZoomPlotWidget(FigureCanvas):
    ...
    def __init__(
        ...
        colors=None,
        all_iscell_t2p=None,
        t2p_match_mat_allday=None,
        track_ops=None,
        imgs=None,
    ):
        nb_plot = len(all_iscell_t2p)
        self.fig, self.ax_zoom = plt.subplots(
            1,
            nb_plot,
            figsize=(10 * nb_plot, nb_plot),
            gridspec_kw={"width_ratios": [1] * nb_plot},
            facecolor="black",
        )
    ...
    def display_zooms(self, selected_cell_index):
        ...
        self.coord_dict = {}
        if (
            self.all_ops is not None
            and self.all_stat_t2p is not None
            and self.all_is_cell is not None
        ):
            for i in range(len(self.imgs)):
                ...
                if self.track_ops.iscell_thr == None:
                    ...
                    indices_lignes_1 = np.where(
                        iscell[:, 1] > self.track_ops.iscell_thr
                    )[0]
                    match_index = self.t2p_match_mat_allday[selected_cell_index, i]
                    true_index = indices_lignes_1[match_index]
