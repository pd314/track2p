...
class DataManagement:
    ...
    def import_files(self, t2p_folder_path, plane, trace_type, channel):
        ...
        track_ops_dict = np.load(
            t2p_folder_path / "track2p" / "track_ops.npy", allow_pickle=True
        ).item()
        ...
        # save track_ops
        np.save(t2p_folder_path / "track2p" / "track_ops.npy", track_ops_dict)
    ...
    def F_processing(
        ...
        if baseline == "maximin":
            ...
            Flow = np.amin(gaussian_filter(Fc, [0.0, sig_baseline]))
