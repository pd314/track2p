...
class FluorescencePlotWidget(FigureCanvas):
    ...
    def draw_rectangle(self):
        ...
        self.rect = patches.Rectangle(
            ...
            linewidth=2,
            edgecolor="white",
        )  # create a rectangle, fill is set to None to make the rectangle transparent
    ...
    def draw_point(self):
        ...
        self.point = self.ax_fluorescence.scatter(
            [self.x0], [self.y0], s=5, color="w"
        )  # Create a new point
        self.draw()
    ...
    def display_all_f_t2p(self, selected_cell_index):
        ...
        if self.all_f_t2p is not None and selected_cell_index is not None:
            ...
            for i, fluorescence_data in list(enumerate(reversed(self.all_f_t2p))):
                ...
                fluorescence_zscore = zscore(
                    fluorescence_data, axis=1, ddof=1
                )  # zscore is used to normalize the fluorescence data
                offset = i * 12  #
                ...
                if len(tstamps) != len(y_values):
                    raise ValueError(
                        f"tstamps and y_values must have the same length, but have lengths {len(tstamps)} and {len(y_values)}"
                    )
                self.ax_fluorescence.plot(
                    tstamps, y_values, label=f"Curve {i + 1}", color=color
                )
