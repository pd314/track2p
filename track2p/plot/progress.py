...
def plot_all_planes(all_ds_avg_ch, track_ops, sat_perc=99, ch="funcional"):
    ...
    fig, axs = plt.subplots(
        ...
        figsize=(3 * len(track_ops.all_ds_path), 3 * nplanes),
        dpi=300,
    )
    ...
    for i in range(nplanes):
        for j in range(len(track_ops.all_ds_path)):
            ...
            axs[i, j].imshow(
                img, cmap="gray", vmin=0, vmax=np.percentile(img, sat_perc)
            )
            axs[i, j].set_title("Plane " + str(i) + " in dataset " + str(j))
            axs[i, j].axis("off")

    fig.savefig(
        track_ops.save_path_fig + f"mean_fov_{ch}.png", bbox_inches="tight", dpi=200
    )
