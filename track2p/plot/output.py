...
def plot_reg_img_output(track_ops):
    ...
    for i in range(nplanes):
        ...
        for j in range(len(track_ops.all_ds_path)):
            img = (
                track_ops.all_ds_avg_ch1[j][i]
                if track_ops.reg_chan == 0
                else track_ops.all_ds_avg_ch2[j][i]
            )
    ...
    for i in range(len(track_ops.all_ds_path) - 1):  # last one won't have overlay
        for j in range(nplanes):
            ...
            row_nonreg = (
                nplanes + 2 * j
            )  # first shift for number of initial rows, then shift by 2 for each plane (before and after registration)
            ...
            mov_img_ref = track_ops.all_ds_mov_img_reg[i][
                j
            ]  # get mov image after registration for this pair
        ...
        if i == 0:
            for k in range(nplanes):
                ...
                axs[nplanes + 2 * k + 1, i].set_ylabel(
                    f"plane{k}\nchan{track_ops.reg_chan}\nreg",
                    rotation=0,
                    size="large",
                    ha="right",
    ...
    axs[2 * nplanes, 0].annotate(
        "",
        xy=(0, 1.1),
        xytext=(n_col + 0.5, 1.1),
    ...
    fig.savefig(
        os.path.join(track_ops.save_path_fig, "reg_img_output.png"),
        bbox_inches="tight",
        dpi=200,
    )
    plt.close(fig)
...
def plot_roi_reg_output(track_ops):
    ...
    for i in range(nplanes):
        ...
        for j in range(len(track_ops.all_ds_path)):
            ...
            axs[i, j].imshow(
                img, cmap="gray", vmin=0, vmax=np.percentile(img, 99), alpha=0.5
            )
            # take the rois of reference unless it is last on, then take the rois of the mov
            all_roi = (
                track_ops.all_ds_all_roi_array_ref[j][i]
                if j < len(track_ops.all_ds_path) - 1
                else track_ops.all_ds_all_roi_array_mov[j - 1][i]
            )
    ...
    for i in range(len(track_ops.all_ds_path) - 1):
        logger.info(
            f"Plotting contours for dataset {i}/{len(track_ops.all_ds_path) - 1}"
        )
        for j in range(track_ops.nplanes):
            ...
            for k in range(all_roi_array_ref.shape[2]):
                axs[row_nonreg, i].contour(
                    all_roi_array_ref[:, :, k], colors="r", linewidths=0.3
                )
            for k in range(all_roi_array_mov.shape[2]):
                axs[row_nonreg, i].contour(
                    all_roi_array_mov[:, :, k], colors="g", linewidths=0.3
                )
            ...
            for k in range(all_roi_array_ref.shape[2]):
                axs[row_reg, i].contour(
                    all_roi_array_ref[:, :, k], colors="r", linewidths=0.3
                )
            for k in range(all_roi_array_reg.shape[2]):
                axs[row_reg, i].contour(
                    all_roi_array_reg[:, :, k], colors="g", linewidths=0.3
                )
    ...
    # save figure into the output path
    fig.savefig(
        track_ops.save_path_fig + "reg_roi_output.png", bbox_inches="tight", dpi=200
    )
    plt.close(fig)
...
def plot_thr_met_hist(all_ds_thr_met, all_ds_thr, track_ops):
    fig, axs = plt.subplots(
        track_ops.nplanes,
        len(all_ds_thr_met),
        figsize=(6 * len(all_ds_thr_met), 6 * track_ops.nplanes),
        sharey=True,
        sharex=True,
    )
    for i in range(len(all_ds_thr_met)):
        for j in range(track_ops.nplanes):
            ...
            this_ax = (
                axs[i]
                if track_ops.nplanes == 1 or len(track_ops.all_ds_path) == 2
                else axs[j][i]
            )
            ...
            this_ax.text(
                all_ds_thr[i][j] + 0.02,
                this_ax.get_ylim()[1] * 0.9,
                f"{track_ops.thr_method} thr.: {all_ds_thr[i][j]:.2f}",
            )
            this_ax.set_title(
                f"ds{i} (ref) to ds{i + 1} (reg); matched {n_abovethr_roi}/{n_reg_roi} ({n_abovethr_roi / n_reg_roi * 100:.1f}%)"
            )
...
def plot_n_matched_roi(all_ds_thr_met, all_ds_thr, track_ops):
    ...
    for i in range(track_ops.nplanes):
        ...
        all_n_abovethr_roi = [
            np.sum(all_ds_thr_met[j][i] > all_ds_thr[j][i])
            for j in range(len(all_ds_thr_met))
        ]
        all_prop_abovethr = [
            n_abovethr_roi / n_reg_roi
            for n_abovethr_roi, n_reg_roi in zip(all_n_abovethr_roi, all_n_reg_roi)
        ]

        ax0 = axs[i, 0] if track_ops.nplanes > 1 else axs[0]

        ax0.bar(
            np.arange(len(all_n_reg_roi)), all_n_reg_roi, color="grey", label="total"
        )
        ax0.bar(
            np.arange(len(all_n_abovethr_roi)),
            all_n_abovethr_roi,
            color="C0",
        ...
        ax1.plot(
            np.arange(len(all_n_reg_roi)), all_prop_abovethr, color="C0", marker="o"
        )
        ...
        fig.savefig(
            os.path.join(track_ops.save_path_fig, "n_prop_matched_roi.png"),
            dpi=200,
            bbox_inches="tight",
        )
        plt.close(fig)
...
def plot_roi_match(
    ...
    for i, nrn_id in enumerate(neuron_ids):
        ...
        if i == 0:
            fig_ref = all_wind_mean_img[
                0
            ]  # reference image for matching histograms along whole image
        else:
            all_wind_mean_img[0] = match_histograms(
                all_wind_mean_img[0], fig_ref
            )  # if not first ROI of whole image then match to first ROI of whole image
    ...
    if ch == 1:
        plt.savefig(
            os.path.join(
                track_ops.save_path_fig,
                f"roi_match_plane{plane_idx}_idx{k}-{k + len(neuron_ids)}.png",
            ),
            dpi=50,
        )
    elif ch == 2:
        plt.savefig(
            os.path.join(
                track_ops.save_path_fig,
                f"roi_match_chan2_plane{plane_idx}_idx{k}-{k + len(neuron_ids)}.png",
            ),
            dpi=50,
        )
    plt.close(fig)
...
def plot_roi_match_multiplane(
    all_ds_mean_img, all_ds_centroids, all_pl_match_mat, track_ops, win_size=48, ch=1
):
...
def plot_allroi_match_multiplane(all_ds_mean_img, all_pl_match_mat, track_ops, ch=1):
    ...
    fig, axs = plt.subplots(
        track_ops.nplanes,
        len(track_ops.all_ds_path),
        figsize=(4 * len(track_ops.all_ds_path), 4 * track_ops.nplanes),
        dpi=300,
    )
    ...
    for plane_idx in range(track_ops.nplanes):
        ...
        for ds_idx, _ in enumerate(track_ops.all_ds_path):
            stat_ds_plane, _ = load_stat_ds_plane(
                track_ops.all_ds_path[ds_idx], track_ops, plane_idx=plane_idx
            )
            ...
            for i, neuron_idx in enumerate(neuron_ids):
                idx_orig = all_pl_match_mat[plane_idx][
                    neuron_idx, ds_idx
                ]  # neurons index in the original recording
                cont = stat_ds_plane

                cont_plot = ax.contour(
                    all_roi_array[:, :, idx_orig],
                    linewidths=0.5,
                    colors=[neuron_colors[i]],
                )
    ...
    if ch == 1:
        plt.savefig(
            os.path.join(track_ops.save_path_fig, f"all_roi_match.png"), dpi=300
        )
    else:
        plt.savefig(
            os.path.join(track_ops.save_path_fig, f"all_roi_match_chan2.png"), dpi=300
        )
    plt.close(fig)
